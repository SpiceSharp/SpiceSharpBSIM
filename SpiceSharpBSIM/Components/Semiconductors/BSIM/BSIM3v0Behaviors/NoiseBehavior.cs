...
{
    /// <summary>
    /// Noise behavior for a <see cref="BSIM3v0"/>.
    /// </summary>
    [BehaviorFor(typeof(BSIM3v0)), AddBehaviorIfNo(typeof(INoiseBehavior))]
    public class NoiseBehavior : FrequencyBehavior, INoiseBehavior
    {
        ...
        void INoiseBehavior.Compute()
        {
            ...
            _rd.Compute(_drainConductance * Parameters.M, _temperature.Temperature);
            _rs.Compute(this._sourceConductance * Parameters.M, _temperature.Temperature);

            if (ModelParameters.NoiMod.Value == 2)
            {
                _id.Compute((this._ueff * Math.Abs((this._qinv * Parameters.M) / (Param.BSIM3v0leff * Param.BSIM3v0leff))), _temperature.Temperature);
            }
            else
            {
                _id.Compute((2.0 / 3.0 * Math.Abs(this._gm + this._gds) * Parameters.M), _temperature.Temperature);
            }
            ...
            if (ModelParameters.NoiMod.Value == 2)
            {
                ...
                {
                    Ssi = StrongInversionNoiseEval3v0(vgs, vds, _state.Point.Value.Frequency, _temperature.Temperature);
                    _flicker.Compute(Ssi);
                }
                else
                {
                    T10 = ModelParameters.OxideTrapDensityA
                        * 8.62e-5 * (_temperature.Temperature + Constants.CelsiusKelvin);
                    T11 = Param.BSIM3v0weff * Parameters.M * Param.BSIM3v0leff
                    ...
                    Slimit = StrongInversionNoiseEval3v0(
                         this._von + 0.1,
                         vds, _state.Point.Value.Frequency, _temperature.Temperature);
                    T1 = Swi + Slimit;
                    if (T1 > 0.0)
            ...
            {
                _flicker.Compute(ModelParameters.Kf *
                      Math.Exp(ModelParameters.Af
                  * Math.Log(Math.Max(Math.Abs(this._cd * Parameters.M),
                  1e-38)))
                  / (Math.Pow(_state.Point.Value.Frequency, ModelParameters.Ef)
                  * Param.BSIM3v0leff
                      * Param.BSIM3v0leff
        ...
        private double StrongInversionNoiseEval3v0(double vgs, double vds, double freq, double temp)
        {
            ...
            if (vds > this._vdsat)
            {
                esat = 2.0 * Param.BSIM3v0vsattemp / this._ueff;
                T0 = ((((vds - this._vdsat) / Param.BSIM3v0litl) + ModelParameters.Em)
                   / esat);
                DelClm = Param.BSIM3v0litl * Math.Log(Math.Max(T0, 1e-38));
            }
            else
                DelClm = 0.0;
            EffFreq = Math.Pow(freq, ModelParameters.Ef);
            T1 = Constants.Charge * Constants.Charge * 8.62e-5 * cd * (temp + Constants.CelsiusKelvin) * this._ueff;
            T2 = 1.0e8 * EffFreq * ModelTemperature.Cox
               * Param.BSIM3v0leff * Param.BSIM3v0leff;
            Vgst = vgs - this._von;
            ...
            T6 = 8.62e-5 * (temp + Constants.CelsiusKelvin) * cd * cd;
            T7 = 1.0e8 * EffFreq * Param.BSIM3v0leff
               * Param.BSIM3v0leff * Param.BSIM3v0weff * Parameters.M;
