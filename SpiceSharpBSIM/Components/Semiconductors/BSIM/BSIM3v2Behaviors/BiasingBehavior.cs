...
{
    /// <summary>
    /// Biasing behavior for a <see cref="BSIM3v2"/>.
    /// </summary>
    [BehaviorFor(typeof(BSIM3v2)), AddBehaviorIfNo(typeof(IBiasingBehavior))]
    [GeneratedParameters]
    public partial class BiasingBehavior : TemperatureBehavior, IBiasingBehavior, ITimeBehavior
    {
        ...
        void IBiasingBehavior.Load()
        {
            ...
            else if (_iteration.Mode == IterationModes.Junction && !Parameters.Off)
            {
                ...
                {
                    vbs = 0.0;
                    vgs = ModelParameters.Type * this.Vth0 + 0.1;
                    vds = 0.1;
                }
            }
                        ...
                        switch (ModelTemperature.IntVersion)
                        {
                            ...
                            case Version.BSIM3v32V322:
                                ...
                                this._cbs = this.IsEvjsm - SourceSatCurrent
                                  + T0 * (vbs - this.Vjsm) + (_iteration.Gmin) * vbs;
                                break;
                            case Version.BSIM3v32V32:
                            default:
                                T0 = (SourceSatCurrent + ModelParameters.Ijth) / Nvtm;
                                this._gbs = T0 + (_iteration.Gmin);
                                this._cbs = ModelParameters.Ijth + (_iteration.Gmin) * vbs
                                  + T0 * (vbs - this.Vjsm);
                                break;
                        }
                    }
                        ...
                        switch (ModelTemperature.IntVersion)
                        {
                            ...
                            case Version.BSIM3v32V322:
                                ...
                                this._cbd = this.IsEvjdm - DrainSatCurrent
                                  + T0 * (vbd - this.Vjdm) + (_iteration.Gmin) * vbd;
                                break;
                            case Version.BSIM3v32V32:
                            default:
                                T0 = (DrainSatCurrent + ModelParameters.Ijth) / Nvtm;
                                this._gbd = T0 + (_iteration.Gmin);
                                this._cbd = ModelParameters.Ijth + (_iteration.Gmin) * vbd
                                  + T0 * (vbd - this.Vjdm);
                                break;
                        }
                    }
            ...
            T0 = Vbs - Param.BSIM3v32vbsc - 0.001;
            T1 = Math.Sqrt(T0 * T0 - 0.004 * Param.BSIM3v32vbsc);
            Vbseff = Param.BSIM3v32vbsc + 0.5 * (T0 + T1);
            ...
            if (Vbseff > 0.0)
            {
                T0 = Param.BSIM3v32phi / (Param.BSIM3v32phi + Vbseff);
                Phis = Param.BSIM3v32phi * T0;
                dPhis_dVb = -T0 * T0;
                sqrtPhis = Param.BSIM3v32phis3 / (Param.BSIM3v32phi + 0.5 * Vbseff);
                dsqrtPhis_dVb = -0.5 * sqrtPhis * sqrtPhis / Param.BSIM3v32phis3;
            }
            else
            {
                Phis = Param.BSIM3v32phi - Vbseff;
                dPhis_dVb = -1.0;
                sqrtPhis = Math.Sqrt(Phis);
                dsqrtPhis_dVb = -0.5 / sqrtPhis;
            }
            Xdep = Param.BSIM3v32Xdep0 * sqrtPhis / Param.BSIM3v32sqrtPhi;
            dXdep_dVb = (Param.BSIM3v32Xdep0 / Param.BSIM3v32sqrtPhi)
                      * dsqrtPhis_dVb;

            Leff = Param.BSIM3v32leff;
            Vtm = ModelTemperature.Vtm;
            /* Vth Calculation */
            T3 = Math.Sqrt(Xdep);
            V0 = Param.BSIM3v32vbi - Param.BSIM3v32phi;

            T0 = Param.BSIM3v32dvt2 * Vbseff;
            if (T0 >= -0.5)
            {
                T1 = 1.0 + T0;
                T2 = Param.BSIM3v32dvt2;
            }
            else /* Added to avoid any discontinuity problems caused by dvt2 */
            {
                T4 = 1.0 / (3.0 + 8.0 * T0);
                T1 = (1.0 + 3.0 * T0) * T4;
                T2 = Param.BSIM3v32dvt2 * T4 * T4;
            }
            lt1 = ModelTemperature.Factor1 * T3 * T1;
            dlt1_dVb = ModelTemperature.Factor1 * (0.5 / T3 * T1 * dXdep_dVb + T3 * T2);

            T0 = Param.BSIM3v32dvt2w * Vbseff;
            if (T0 >= -0.5)
            {
                T1 = 1.0 + T0;
                T2 = Param.BSIM3v32dvt2w;
            }
            else /* Added to avoid any discontinuity problems caused by dvt2w */
            {
                T4 = 1.0 / (3.0 + 8.0 * T0);
                T1 = (1.0 + 3.0 * T0) * T4;
                T2 = Param.BSIM3v32dvt2w * T4 * T4;
            }
            ltw = ModelTemperature.Factor1 * T3 * T1;
            dltw_dVb = ModelTemperature.Factor1 * (0.5 / T3 * T1 * dXdep_dVb + T3 * T2);

            T0 = -0.5 * Param.BSIM3v32dvt1 * Leff / lt1;
            if (T0 > -EXP_THRESHOLD)
            {
                T1 = Math.Exp(T0);
            ...
            this._thetavth = Param.BSIM3v32dvt0 * Theta0;
            Delt_vth = this._thetavth * V0;
            dDelt_vth_dVb = Param.BSIM3v32dvt0 * dTheta0_dVb * V0;

            T0 = -0.5 * Param.BSIM3v32dvt1w * Param.BSIM3v32weff * Leff / ltw;
            if (T0 > -EXP_THRESHOLD)
            {
                T1 = Math.Exp(T0);
                T2 = T1 * (1.0 + 2.0 * T1);
            ...
            T2 = T0 * V0;
            dT2_dVb = Param.BSIM3v32dvt0w * dT2_dVb * V0;

            TempRatio = _temperature.Temperature / ModelParameters.Tnom - 1.0;
            T0 = Math.Sqrt(1.0 + Param.BSIM3v32nlx / Leff);
            T1 = Param.BSIM3v32k1ox * (T0 - 1.0) * Param.BSIM3v32sqrtPhi
               + (Param.BSIM3v32kt1 + Param.BSIM3v32kt1l / Leff
               + Param.BSIM3v32kt2 * Vbseff) * TempRatio;
            tmp2 = ModelParameters.Tox * Param.BSIM3v32phi
                 / (Param.BSIM3v32weff + Param.BSIM3v32w0);

            T3 = Param.BSIM3v32eta0 + Param.BSIM3v32etab * Vbseff;
            ...
            dDIBL_Sft_dVd = T3 * Param.BSIM3v32theta0vb0;
            DIBL_Sft = dDIBL_Sft_dVd * Vds;

            Vth = ModelParameters.Type * this.Vth0 - Param.BSIM3v32k1
                * Param.BSIM3v32sqrtPhi + Param.BSIM3v32k1ox * sqrtPhis
                - Param.BSIM3v32k2ox * Vbseff - Delt_vth - T2 + (Param.BSIM3v32k3
                + Param.BSIM3v32k3b * Vbseff) * tmp2 + T1 - DIBL_Sft;

            this._von = Vth;

            dVth_dVb = Param.BSIM3v32k1ox * dsqrtPhis_dVb - Param.BSIM3v32k2ox
                     - dDelt_vth_dVb - dT2_dVb + Param.BSIM3v32k3b * tmp2
                     - Param.BSIM3v32etab * Vds * Param.BSIM3v32theta0vb0 * T4
                     + Param.BSIM3v32kt2 * TempRatio;
            dVth_dVd = -dDIBL_Sft_dVd;

            /* Calculate n */
            tmp2 = Param.BSIM3v32nfactor * EPSSI / Xdep;
            tmp3 = Param.BSIM3v32cdsc + Param.BSIM3v32cdscb * Vbseff
                 + Param.BSIM3v32cdscd * Vds;
            tmp4 = (tmp2 + tmp3 * Theta0 + Param.BSIM3v32cit) / ModelTemperature.Cox;
            if (tmp4 >= -0.5)
            {
                n = 1.0 + tmp4;
                dn_dVb = (-tmp2 / Xdep * dXdep_dVb + tmp3 * dTheta0_dVb
                       + Param.BSIM3v32cdscb * Theta0) / ModelTemperature.Cox;
                dn_dVd = Param.BSIM3v32cdscd * Theta0 / ModelTemperature.Cox;
            }
            else
            /* avoid  discontinuity problems caused by tmp4 */
            {
                ...
                T0 *= T0;
                dn_dVb = (-tmp2 / Xdep * dXdep_dVb + tmp3 * dTheta0_dVb
                       + Param.BSIM3v32cdscb * Theta0) / ModelTemperature.Cox * T0;
                dn_dVd = Param.BSIM3v32cdscd * Theta0 / ModelTemperature.Cox * T0;
            }

            /* Poly Gate Si Depletion Effect */
            T0 = this.Vfb + Param.BSIM3v32phi;
            if ((Param.BSIM3v32ngate > 1.0e18) && (Param.BSIM3v32ngate < 1.0e25)
                 && (Vgs > T0))
            /* added to avoid the problem caused by ngate */
            {
                T1 = 1.0e6 * Charge_q * EPSSI * Param.BSIM3v32ngate
            ...
            T10 = 2.0 * n * Vtm;
            VgstNVt = Vgst / T10;
            ExpArg = (2.0 * Param.BSIM3v32voff - Vgst) / T10;
            ...
            {
                T0 = (Vgst - Param.BSIM3v32voff) / (n * Vtm);
                ExpVgst = Math.Exp(T0);
                Vgsteff = Vtm * Param.BSIM3v32cdep0 / ModelTemperature.Cox * ExpVgst;
            ...
            else
            {
                ...
                dT2_dVg = -ModelTemperature.Cox / (Vtm * Param.BSIM3v32cdep0)
                        * Math.Exp(ExpArg);
                T2 = 1.0 - T10 * dT2_dVg;
            ...
            /* Added revision dependent code */
            if (ModelTemperature.IntVersion > Version.BSIM3v32V323)
            {
                this._vgsteff = Vgsteff;
            }

            /* Calculate Effective Channel Geometry */
            T9 = sqrtPhis - Param.BSIM3v32sqrtPhi;
            Weff = Param.BSIM3v32weff - 2.0 * (Param.BSIM3v32dwg * Vgsteff
                 + Param.BSIM3v32dwb * T9);
            dWeff_dVg = -2.0 * Param.BSIM3v32dwg;
            dWeff_dVb = -2.0 * Param.BSIM3v32dwb * dsqrtPhis_dVb;
            ...
            T0 = Param.BSIM3v32prwg * Vgsteff + Param.BSIM3v32prwb * T9;
            if (T0 >= -0.9)
            {
                Rds = Param.BSIM3v32rds0 * (1.0 + T0);
                dRds_dVg = Param.BSIM3v32rds0 * Param.BSIM3v32prwg;
                dRds_dVb = Param.BSIM3v32rds0 * Param.BSIM3v32prwb * dsqrtPhis_dVb;
            }
            else
            /* to avoid the discontinuity problem due to prwg and prwb*/
            {
                T1 = 1.0 / (17.0 + 20.0 * T0);
                Rds = Param.BSIM3v32rds0 * (0.8 + T0) * T1;
                T1 *= T1;
                dRds_dVg = Param.BSIM3v32rds0 * Param.BSIM3v32prwg * T1;
                dRds_dVb = Param.BSIM3v32rds0 * Param.BSIM3v32prwb * dsqrtPhis_dVb
                         * T1;
            }
            /* Added revision dependent code */
            ...
            /* Calculate Abulk */
            T1 = 0.5 * Param.BSIM3v32k1ox / sqrtPhis;
            dT1_dVb = -T1 / sqrtPhis * dsqrtPhis_dVb;

            T9 = Math.Sqrt(Param.BSIM3v32xj * Xdep);
            tmp1 = Leff + 2.0 * T9;
            T5 = Leff / tmp1;
            tmp2 = Param.BSIM3v32a0 * T5;
            tmp3 = Param.BSIM3v32weff + Param.BSIM3v32b1;
            tmp4 = Param.BSIM3v32b0 / tmp3;
            T2 = tmp2 + tmp4;
            ...
            T8 = Param.BSIM3v32ags * Param.BSIM3v32a0 * T7;
            dAbulk_dVg = -T1 * T8;
            ...
            /* added to avoid the problems caused by Abulk */
            {
                ...
                Abulk = (0.2 - Abulk) * T9;
                /* Added revision dependent code */
                if (ModelTemperature.IntVersion > Version.BSIM3v32V32)
                {
                    T10 = T9 * T9;
                    dAbulk_dVb *= T10;
                    dAbulk_dVg *= T10;
            ...
            T2 = Param.BSIM3v32keta * Vbseff;
            if (T2 >= -0.9)
            {
                T0 = 1.0 / (1.0 + T2);
                dT0_dVb = -Param.BSIM3v32keta * T0 * T0;
            }
            else
            /* added to avoid the problems caused by Keta */
            {
                T1 = 1.0 / (0.8 + T2);
                T0 = (17.0 + 20.0 * T2) * T1;
                dT0_dVb = -Param.BSIM3v32keta * T1 * T1;
            }
            dAbulk_dVg *= T0;
            ...
            {
                T0 = Vgsteff + Vth + Vth;
                T2 = Param.BSIM3v32ua + Param.BSIM3v32uc * Vbseff;
                T3 = T0 / ModelParameters.Tox;
                T5 = T3 * (T2 + Param.BSIM3v32ub * T3);
                dDenomi_dVg = (T2 + 2.0 * Param.BSIM3v32ub * T3) / ModelParameters.Tox;
                dDenomi_dVd = dDenomi_dVg * 2.0 * dVth_dVd;
                dDenomi_dVb = dDenomi_dVg * 2.0 * dVth_dVb + Param.BSIM3v32uc * T3;
            }
            else if (ModelParameters.MobMod.Value == 2)
            {
                T5 = Vgsteff / ModelParameters.Tox * (Param.BSIM3v32ua
                   + Param.BSIM3v32uc * Vbseff + Param.BSIM3v32ub * Vgsteff
                   / ModelParameters.Tox);
                dDenomi_dVg = (Param.BSIM3v32ua + Param.BSIM3v32uc * Vbseff
                            + 2.0 * Param.BSIM3v32ub * Vgsteff / ModelParameters.Tox)
                            / ModelParameters.Tox;
                dDenomi_dVd = 0.0;
                dDenomi_dVb = Vgsteff * Param.BSIM3v32uc / ModelParameters.Tox;
            }
            else
            {
                T0 = Vgsteff + Vth + Vth;
                T2 = 1.0 + Param.BSIM3v32uc * Vbseff;
                T3 = T0 / ModelParameters.Tox;
                T4 = T3 * (Param.BSIM3v32ua + Param.BSIM3v32ub * T3);
                T5 = T4 * T2;
                dDenomi_dVg = (Param.BSIM3v32ua + 2.0 * Param.BSIM3v32ub * T3) * T2
                            / ModelParameters.Tox;
                dDenomi_dVd = dDenomi_dVg * 2.0 * dVth_dVd;
                dDenomi_dVb = dDenomi_dVg * 2.0 * dVth_dVb + Param.BSIM3v32uc * T4;
            }
            ...
            WVCox = Weff * Param.BSIM3v32vsattemp * ModelTemperature.Cox;
            WVCoxRds = WVCox * Rds;

            Esat = 2.0 * Param.BSIM3v32vsattemp / ueff;
            EsatL = Esat * Leff;
            T0 = -EsatL / ueff;
            ...
            {
                T0 = 1.0 - Param.BSIM3v32a2;
                T1 = T0 - Param.BSIM3v32a1 * Vgsteff - 0.0001;
                T2 = Math.Sqrt(T1 * T1 + 0.0004 * T0);
                Lambda = Param.BSIM3v32a2 + T0 - 0.5 * (T1 + T2);
                dLambda_dVg = 0.5 * Param.BSIM3v32a1 * (1.0 + T1 / T2);
            }
            else
            {
                T1 = Param.BSIM3v32a2 + Param.BSIM3v32a1 * Vgsteff - 0.0001;
                T2 = Math.Sqrt(T1 * T1 + 0.0004 * Param.BSIM3v32a2);
                Lambda = 0.5 * (T1 + T2);
                dLambda_dVg = 0.5 * Param.BSIM3v32a1 * (1.0 + T1 / T2);
            }
            ...
            T1 = Vdsat - Vds - Param.BSIM3v32delta;
            dT1_dVg = dVdsat_dVg;
            dT1_dVd = dVdsat_dVd - 1.0;
            dT1_dVb = dVdsat_dVb;

            T2 = Math.Sqrt(T1 * T1 + 4.0 * Param.BSIM3v32delta * Vdsat);
            T0 = T1 / T2;
            T3 = 2.0 * Param.BSIM3v32delta / T2;
            ...
            /* Calculate VACLM */
            if ((Param.BSIM3v32pclm > 0.0) && (diffVds > 1.0e-10))
            {
                T0 = 1.0 / (Param.BSIM3v32pclm * Abulk * Param.BSIM3v32litl);
            ...
            /* Calculate VADIBL */
            if (Param.BSIM3v32thetaRout > 0.0)
            {
                T8 = Abulk * Vdsat;
                T0 = Vgst2Vtm * T8;
                ...
                T9 = T1 * T1;
                T2 = Param.BSIM3v32thetaRout;
                VADIBL = (Vgst2Vtm - T0 / T1) / T2;
                dVADIBL_dVg = (1.0 - dT0_dVg / T1 + T0 * dT1_dVg / T9) / T2;
                dVADIBL_dVb = (-dT0_dVb / T1 + T0 * dT1_dVb / T9) / T2;
                dVADIBL_dVd = (-dT0_dVd / T1 + T0 * dT1_dVd / T9) / T2;

                T7 = Param.BSIM3v32pdiblb * Vbseff;
                if (T7 >= -0.9)
                {
                    T3 = 1.0 / (1.0 + T7);
                    VADIBL *= T3;
                    dVADIBL_dVg *= T3;
                    dVADIBL_dVb = (dVADIBL_dVb - VADIBL * Param.BSIM3v32pdiblb)
                                * T3;
                    dVADIBL_dVd *= T3;
                }
                else
                /* Added to avoid the discontinuity problem caused by pdiblcb */
                {
                    ...
                    dVADIBL_dVb = dVADIBL_dVb * T3
                                - VADIBL * Param.BSIM3v32pdiblb * T4 * T4;
                    dVADIBL_dVd *= T3;
                    VADIBL *= T3;
                }
            }
            ...
            {
                if (diffVds > Param.BSIM3v32pscbe1 * Param.BSIM3v32litl
                    / EXP_THRESHOLD)
                {
                    T0 = Param.BSIM3v32pscbe1 * Param.BSIM3v32litl / diffVds;
                    VASCBE = Leff * Math.Exp(T0) / Param.BSIM3v32pscbe2;
                ...
                {
                    VASCBE = MAX_EXP * Leff / Param.BSIM3v32pscbe2;
            ...
            tmp = Param.BSIM3v32alpha0 + Param.BSIM3v32alpha1 * Leff;
            if ((tmp <= 0.0) || (Param.BSIM3v32beta0 <= 0.0))
            {
            ...
            {
                T2 = tmp / Leff;
                if (diffVds > Param.BSIM3v32beta0 / EXP_THRESHOLD)
                {
                    T0 = -Param.BSIM3v32beta0 / diffVds;
             ...
             * 0, 1, 2 & 3 stored in this._qinv 1/1998 */
            ...
            else if (ModelParameters.CapMod.Value == 0)
            {
                ...
                {
                    Vbseff = Param.BSIM3v32phi - Phis;
                    dVbseff_dVb = -dPhis_dVb;
                }

                Vfb = Param.BSIM3v32vfbcv;
                Vth = Vfb + Param.BSIM3v32phi + Param.BSIM3v32k1ox * sqrtPhis;
                Vgst = Vgs_eff - Vth;
                dVth_dVb = Param.BSIM3v32k1ox * dsqrtPhis_dVb;
                dVgst_dVb = -dVth_dVb;
                ...
                {
                    T1 = 0.5 * Param.BSIM3v32k1ox;
                    T2 = Math.Sqrt(T1 * T1 + Arg1);
                    qgate = CoxWL * Param.BSIM3v32k1ox * (T2 - T1);
                    qbulk = -qgate;
                    qdrn = 0.0;
                ...
                else
                {
                    ...
                    AbulkCV = Abulk0 * Param.BSIM3v32abulkCVfactor;
                    dAbulkCV_dVb = Param.BSIM3v32abulkCVfactor * dAbulk0_dVb;
                    Vdsat = Vgst / AbulkCV;
                    ...
                    {   /* 40/60 Charge partition model */
                        ...
                        {   /* linear region  */
                            ...
                            T9 = 0.25 * CoxWL;
                            T4 = T9 * Alphaz;
                            qgate = CoxWL * (Vgs_eff - Vfb - Param.BSIM3v32phi
                                  - 0.5 * (Vds - T3));
            ...
            else
            {
                ...
                {
                    VbseffCV = Param.BSIM3v32phi - Phis;
                ...
                noff = n * Param.BSIM3v32noff;
                dnoff_dVd = Param.BSIM3v32noff * dn_dVd;
                dnoff_dVb = Param.BSIM3v32noff * dn_dVb;
                T0 = Vtm * noff;
                voffcv = Param.BSIM3v32voffcv;
                ...
                if (ModelParameters.CapMod.Value == 1)
                {
                    ...
                    switch (ModelTemperature.IntVersion)
                    {
                        ...
                        case Version.BSIM3v32V32:
                            ...
                            break;
                        default:
                            Vfb = Vth - Param.BSIM3v32phi - Param.BSIM3v32k1ox * sqrtPhis;
                            dVfb_dVb = dVth_dVb - Param.BSIM3v32k1ox * dsqrtPhis_dVb;
                            dVfb_dVd = dVth_dVd;
                            break;
                    }
                    ...
                    {
                        T0 = 0.5 * Param.BSIM3v32k1ox;
                        T1 = Math.Sqrt(T0 * T0 + Arg1);
                        T2 = CoxWL * T0 / T1;

                        qgate = CoxWL * Param.BSIM3v32k1ox * (T1 - T0);
                ...
                else if (ModelParameters.CapMod.Value == 2)
                {
                    ...
                    Qsub0 = CoxWL * Param.BSIM3v32k1ox * (T1 - T0);
                ...
                else if (ModelParameters.CapMod.Value == 3)
                {
                    ...
                    tmp = T0 * Param.BSIM3v32acde;
                    if ((-EXP_THRESHOLD < tmp) && (tmp < EXP_THRESHOLD))
                    {
                        Tcen = Param.BSIM3v32ldeb * Math.Exp(tmp);
                        dTcen_dVg = Param.BSIM3v32acde * Tcen;
                        dTcen_dVb = dTcen_dVg * dT0_dVb;
                        dTcen_dVg *= dT0_dVg;
                    }
                    else if (tmp <= -EXP_THRESHOLD)
                    {
                        Tcen = Param.BSIM3v32ldeb * MIN_EXP;
                        dTcen_dVg = dTcen_dVb = 0.0;
                    }
                    else
                    {
                        Tcen = Param.BSIM3v32ldeb * MAX_EXP;
                        dTcen_dVg = dTcen_dVb = 0.0;
                    }

                    LINK = 1.0e-3 * ModelParameters.Tox;
                    V3 = Param.BSIM3v32ldeb - Tcen - LINK;
                    V4 = Math.Sqrt(V3 * V3 + 4.0 * LINK * Param.BSIM3v32ldeb);
                    Tcen = Param.BSIM3v32ldeb - 0.5 * (V3 + V4);
                    T1 = 0.5 * (1.0 + V3 / V4);
                    dTcen_dVg *= T1;
                    dTcen_dVb *= T1;
                    ...
                    Qsub0 = CoxWLcen * Param.BSIM3v32k1ox * (T1 - T0);
                    QovCox = Qsub0 / Coxeff;
                    dQsub0_dVg = T2 * (dVgs_eff_dVg - dVfbeff_dVg - dVgsteff_dVg)
                               + QovCox * dCoxeff_dVg;
                    dQsub0_dVd = -T2 * dVgsteff_dVd;
                    ...
                    if (Param.BSIM3v32k1ox <= 0.0)
                    {
                        Denomi = 0.25 * Param.BSIM3v32moin * Vtm;
                        T0 = 0.5 * Param.BSIM3v32sqrtPhi;
                    }
                    else
                    {
                        Denomi = Param.BSIM3v32moin * Vtm
                               * Param.BSIM3v32k1ox * Param.BSIM3v32k1ox;
                        T0 = Param.BSIM3v32k1ox * Param.BSIM3v32sqrtPhi;
                    }
                    T1 = 2.0 * T0 + Vgsteff;
                    ...
                    T3 = 4.0 * (Vth - this.Vfbzb - Param.BSIM3v32phi);
                    Tox += Tox;
                    if (T3 >= 0.0)
                    {
                        /* Added revision dependent code */
        ...
        finished:
                        ...
                        switch (ModelTemperature.IntVersion)
                        {
                            ...
                            case Version.BSIM3v32V323:
                                czbdsw = ModelTemperature.UnitLengthSidewallTempJctCap
                                       * (Parameters.DrainPerimeter - Param.BSIM3v32weff);
                                czbdswg = ModelTemperature.UnitLengthGateSidewallTempJctCap
                                        * Param.BSIM3v32weff;
                                break;
                            case Version.BSIM3v32V322:
                        ...
                        switch (ModelTemperature.IntVersion)
                        {
                            ...
                            case Version.BSIM3v32V323:
                                czbssw = ModelTemperature.UnitLengthSidewallTempJctCap
                                  * (Parameters.SourcePerimeter - Param.BSIM3v32weff);
                                czbsswg = ModelTemperature.UnitLengthGateSidewallTempJctCap
                                  * Param.BSIM3v32weff;
                                break;
                            case Version.BSIM3v32V322:
                    ...
                    switch (ModelTemperature.IntVersion)
                    {
                        ...
                        case Version.BSIM3v32V323:
                            ACM.JunctionCapacitances(
                                ...
                                ModelTemperature.UnitLengthGateSidewallTempJctCap,
                                out czbd,
                                out czbdsw,
                                out czbdswg,
                                out czbs,
                        ...
                        default:
                            ACM.JunctionCapacitances(
                                ...
                                Parameters.SourceArea,
                                Parameters.SourcePerimeter,
                                ModelParameters.UnitAreaJctCap,
                                ModelParameters.UnitLengthSidewallJctCap,
                                ModelParameters.UnitLengthGateSidewallJctCap,
                                out czbd,
                                out czbdsw,
                                out czbdswg,
                                out czbs,
            ...
            {
                /* code merge -JX */
                cgdo = Param.BSIM3v32cgdo;
                qgdo = Param.BSIM3v32cgdo * vgd;
                cgso = Param.BSIM3v32cgso;
                qgso = Param.BSIM3v32cgso * vgs;
            }
            else if (ModelParameters.CapMod.Value == 1)
            {
                if (vgd < 0.0)
                {
                    T1 = Math.Sqrt(1.0 - 4.0 * vgd / Param.BSIM3v32ckappa);
                    cgdo = Param.BSIM3v32cgdo + Param.BSIM3v32weffCV
                         * Param.BSIM3v32cgdl / T1;
                    qgdo = Param.BSIM3v32cgdo * vgd - Param.BSIM3v32weffCV * 0.5
                         * Param.BSIM3v32cgdl * Param.BSIM3v32ckappa * (T1 - 1.0);
                }
                else
                {
                    cgdo = Param.BSIM3v32cgdo + Param.BSIM3v32weffCV
                         * Param.BSIM3v32cgdl;
                    qgdo = (Param.BSIM3v32weffCV * Param.BSIM3v32cgdl
                         + Param.BSIM3v32cgdo) * vgd;
                }

                if (vgs < 0.0)
                {
                    T1 = Math.Sqrt(1.0 - 4.0 * vgs / Param.BSIM3v32ckappa);
                    cgso = Param.BSIM3v32cgso + Param.BSIM3v32weffCV
                         * Param.BSIM3v32cgsl / T1;
                    qgso = Param.BSIM3v32cgso * vgs - Param.BSIM3v32weffCV * 0.5
                         * Param.BSIM3v32cgsl * Param.BSIM3v32ckappa * (T1 - 1.0);
                }
                else
                {
                    cgso = Param.BSIM3v32cgso + Param.BSIM3v32weffCV
                         * Param.BSIM3v32cgsl;
                    qgso = (Param.BSIM3v32weffCV * Param.BSIM3v32cgsl
                         + Param.BSIM3v32cgso) * vgs;
                }
            }
            else
            {
                ...
                T3 = Param.BSIM3v32weffCV * Param.BSIM3v32cgdl;
                T4 = Math.Sqrt(1.0 - 4.0 * T2 / Param.BSIM3v32ckappa);
                cgdo = Param.BSIM3v32cgdo + T3 - T3 * (1.0 - 1.0 / T4)
                     * (0.5 - 0.5 * T0 / T1);
                qgdo = (Param.BSIM3v32cgdo + T3) * vgd - T3 * (T2
                     + 0.5 * Param.BSIM3v32ckappa * (T4 - 1.0));

                T0 = vgs + DELTA_1;
                T1 = Math.Sqrt(T0 * T0 + 4.0 * DELTA_1);
                T2 = 0.5 * (T0 - T1);
                T3 = Param.BSIM3v32weffCV * Param.BSIM3v32cgsl;
                T4 = Math.Sqrt(1.0 - 4.0 * T2 / Param.BSIM3v32ckappa);
                cgso = Param.BSIM3v32cgso + T3 - T3 * (1.0 - 1.0 / T4)
                     * (0.5 - 0.5 * T0 / T1);
                qgso = (Param.BSIM3v32cgso + T3) * vgs - T3 * (T2
                     + 0.5 * Param.BSIM3v32ckappa * (T4 - 1.0));
            }
            ...
            if (this._mode > 0)
            {
                if (Parameters.NqsMod.Value == 0)
                {
                    ...
                    gcbgb = (this._cbgb - Param.BSIM3v32cgbo) * ag0;
                    gcbdb = (this._cbdb - this._capbd) * ag0;
                    gcbsb = (this._cbsb - this._capbs) * ag0;

                    qgd = qgdo;
                    qgs = qgso;
                    qgb = Param.BSIM3v32cgbo * vgb;
                    qgate += qgd + qgs + qgb;
                    qbulk -= qgb;
                    qdrn -= qgd;
                ...
                else
                {
                    ...
                    gcggb = (cgdo + cgso + Param.BSIM3v32cgbo) * ag0;
                    gcgdb = -cgdo * ag0;
                    gcgsb = -cgso * ag0;
        ...
        line850:
            ...
            if (Parameters.NqsMod.Value != 0)
                this._gtau = 16.0 * this.U0temp * ModelTemperature.Vtm
                                / Param.BSIM3v32leffCV / Param.BSIM3v32leffCV
                                * ScalingFactor;
            else
                this._gtau = 0.0;
        ...
        line900:
            ...
            if (this._mode >= 0)
            {
                ...
                ceqbd = -ModelParameters.Type * (this._csub
            ...
            else
            {
                ...
                ceqbs = -ModelParameters.Type * (this._csub
